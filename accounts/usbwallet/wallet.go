...
type wallet struct {
	...
	deriveChain    tomochain.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain tomochain.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
