...
func (s *Service) loop() {
	...
	HandleLoop:
		for {
			select {
			// AddNotify of chain head events, but drop if too frequent
			...
			// AddNotify of new transaction events, but drop if too frequent
			case <-txEventCh:
...
func (s *Service) assembleBlockStats(block *types.Block) *blockStats {
	...
	if s.eth != nil {
		...
		if block == nil {
			block = s.eth.BlockChain().GetCurrentBlock()
		}
		header = block.Header()
...
func (s *Service) reportPending(conn *websocket.Conn) error {
	...
	var pending int
	if s.eth != nil {
		pending, _ = s.eth.GetTxPool().Stats()
	} else {
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	if s.eth != nil {
		mining = s.eth.GetMiner().Mining()
		hashrate = int(s.eth.GetMiner().HashRate())
