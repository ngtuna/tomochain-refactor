...
type BlockChain interface {
	...
	// GetCurrentBlock retrieves the head block from the local chain.
	GetCurrentBlock() *types.Block

	// GetCurrentFastBlock retrieves the head fast block from the local chain.
	GetCurrentFastBlock() *types.Block
...
func (d *Downloader) Progress() ethereum.SyncProgress {
	...
	switch d.mode {
	case FullSync:
		current = d.blockchain.GetCurrentBlock().NumberU64()
	case FastSync:
		current = d.blockchain.GetCurrentFastBlock().NumberU64()
	case LightSync:
...
func (d *Downloader) findAncestor(p *peerConnection, height uint64) (uint64, error) {
	...
	if d.mode == FullSync {
		ceil = d.blockchain.GetCurrentBlock().NumberU64()
	} else if d.mode == FastSync {
		ceil = d.blockchain.GetCurrentFastBlock().NumberU64()
	}
		...
		case packet := <-d.headerCh:
			...
			for i := 0; i < len(headers); i++ {
				if number := headers[i].Number.Int64(); number != from+int64(i)*16 {
					p.log.Warn("GetHead headers broke chain ordering", "index", i, "requested", from+int64(i)*16, "received", number)
...
func (d *Downloader) processHeaders(origin uint64, pivot uint64, td *big.Int) error {
	...
	defer func() {
		if len(rollback) > 0 {
			...
			if d.mode != LightSync {
				lastFastBlock = d.blockchain.GetCurrentFastBlock().Number()
				lastBlock = d.blockchain.GetCurrentBlock().Number()
			}
			d.lightchain.Rollback(hashes)
			curFastBlock, curBlock := common.Big0, common.Big0
			if d.mode != LightSync {
				curFastBlock = d.blockchain.GetCurrentFastBlock().Number()
				curBlock = d.blockchain.GetCurrentBlock().Number()
			}
	...
	for {
		...
		case headers := <-d.headerProcCh:
			...
			if len(headers) == 0 {
				// AddNotify everyone that headers are fully processed
				for _, ch := range []chan bool{d.bodyWakeCh, d.receiptWakeCh} {
					select {
					case ch <- false:
					case <-d.cancelCh:
				...
				// R: Nothing to give
				if d.mode != LightSync {
					head := d.blockchain.GetCurrentBlock()
