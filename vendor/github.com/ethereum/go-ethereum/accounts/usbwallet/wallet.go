...
func (w *wallet) Open(passphrase string) error {
	...
	// AddNotify anyone listening for wallet events that a new device is accessible
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		// AddNotify the user of termination and loop after a bit of time (to avoid trashing)
		reqc <- struct{}{}
		if err == nil {
			select {
			case errc = <-w.deriveQuit:
